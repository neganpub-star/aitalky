...
- 前端 `aitalky-web/`:pnpm monorepo,**三个端都在 `apps/` 下**(不是三个独立仓库):`apps/console`(坐席台,React+TS+Vite+AntD)、`apps/messenger`(信使端 H5,终端客户聊天窗,轻量无 AntD)、后续 `apps/admin`(平台后管)。共享依赖/工具链/可抽 `packages/*` 复用。
...
## 8. 铁律:实时消息绝不靠 WS 保证完整(避开 cicada 坑)
> cicada 的坑:WS 弱网/断连漏帧后,消息其实已落 Mongo **没丢**,但前端把 WS 当成消息流唯一来源、**重连不自动对账**,非得用户**手动刷新/重开会话**才显示。新系统**严禁**重蹈。详见 `doc/design/消息可靠投递与漏消息补偿.md`。
- **WS 只当"通知",seq 当"真相"**:消息完整性靠「会话内单调 seq + `sync(afterSeq)` 增量拉 + 会话表权威 `last_seq`/`unread_count`」,不靠 WS 累积。
- **全自动无感补偿,用户永不刷新**:重连 onopen 自动 sync、WS 帧 gap 检测、列表/详情回带 `lastSeq` 做 `serverLastSeq>localMaxSeq` 对账、focus 补拉、(可选)pong 夹带 lastSeq。
- **seq 空洞容忍铁律**:seq 先分配后落库,落库失败留永久空洞;前端 `localMaxSeq` **以实际 max(seq) 前进、绝不死等 +1 连续**,gap 只触发一次 sync 不阻塞推进。
- **按 seq 有序插入(非 append 到底)**;**未读/预览以服务端 DB 为权威**(多端一致,本地累加仅即时体验)。
