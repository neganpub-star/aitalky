    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        {plans.map((p) => {
          ...
          return (
            <div key={p.id} style={{
              position: 'relative', width: 240, borderRadius: 12, overflow: 'hidden',
              ...
              {/* 当前订阅角标 */}
              {isCurrent && (
                <div style={{ position: 'absolute', top: 14, right: -28, transform: 'rotate(45deg)', background: token.colorPrimary, color: '#fff', fontSize: 11, padding: '2px 30px' }}>
                  {t('bill.current')}
                </div>
              )}
              <div style={{ padding: '20px 18px' }}>
                ...
                <div style={{ margin: '16px 0' }}>
                  {p.isCustom ? (
                    <span style={{ fontSize: 26, fontWeight: 700, color: token.colorTextTertiary }}>{t('bill.customPrice')}</span>
                  ) : (
                    <><span style={{ fontSize: 30, fontWeight: 700 }}>${Number(p.monthlyPrice)}</span>
                      <span style={{ fontSize: 13, color: token.colorTextTertiary }}> {t('bill.perMonth')}</span></>
                  )}
                </div>
                {/* 订阅/续费按钮(下单弹窗第③期;本期占位提示) */}
                <Button type={isCurrent ? 'primary' : 'default'} block
                  onClick={() => message.info(t('settings.wip'))}>
                  {p.isCustom ? t('bill.customPrice') : isCurrent ? t('bill.renewUpgrade') : t('bill.subscribe')}
                </Button>
                {/* 套餐服务 */}
                <div style={{ marginTop: 20, fontSize: 13, color: token.colorTextTertiary }}>{t('bill.planService')}</div>
