-- ============================================================
-- aitalky V1 数据库 DDL (MySQL 8) —— 已据 ByteTrack 文档逐篇评审修订
-- 约定:
--   1) 主键 id = 雪花ID(bigint); 2) 租户业务表带 project_id; 平台表(pf_)/全局表除外
--   3) 每表统一: create_by/create_time/update_by/update_time + del_flag
--   4) 金额 decimal; 字符集 utf8mb4 + 排序规则 utf8mb4_general_ci; 中文注释; 消息体在 MongoDB
--
-- 角色/分层: 平台后管(pf_admin 登录) / 租户·坐席(id_account 登录) / 终端客户(无登录)
-- 资源类型 resource_type(配额/加量包/用量统一用): seat 席位 / translate_char 翻译字符 / customer 客户配额  (V2再加 article/site/ai_token)
--
-- 前缀: pf_平台 | id_身份租户 | mse_信使 | asn_分配 | cus_客户 | cnv_会话 | sup_辅助 | pay_计费
-- ============================================================
